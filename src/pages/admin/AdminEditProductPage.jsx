...
import { Save, ArrowLeft, ImagePlus, Trash2, Tags, AlertTriangle } from 'lucide-react';
...
const AdminEditProductPage = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        const fetchedProduct = await productService.getProductById(productId, admin.id);
        if (fetchedProduct) {
          setProductData({
            ...fetchedProduct,
            price: String(fetchedProduct.price).replace('.',','), 
            tags: Array.isArray(fetchedProduct.tags) ? fetchedProduct.tags.join(', ') : '',
            images: fetchedProduct.images || [],
          });
        } else {
          toast({ title: 'Erro', description: 'Produto não encontrado ou não pertence a este administrador.', variant: 'destructive' });
    ...
    fetchProduct();
  }, [productId, admin, toast, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast({ title: 'Produto Atualizado!', description: `${productData.name} foi atualizado com sucesso.` });
      navigate('/admin/produtos');
    } catch (error) {
  ...
  if (isFetching) {
    return <div className="flex justify-center items-center h-screen text-polly-text dark:text-polly-white">Carregando dados do produto...</div>;
  }
  
  if (!productData.name && !isFetching) { 
    return (
      <div className="flex flex-col justify-center items-center h-screen text-polly-text dark:text-polly-white">
        <AlertTriangle className="w-16 h-16 text-red-500 mb-4" />
        <p className="text-xl mb-4">Produto não encontrado ou erro ao carregar.</p>
        <Button onClick={() => navigate('/admin/produtos')} className="btn-primary">
  ...
  return (
    ...
    >
      ...
      <Card className="shadow-xl bg-white dark:bg-polly-blue-dark/20 dark:border-polly-blue/30">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-polly-blue-dark dark:text-polly-blue">Editar Produto</CardTitle>
          <CardDescription className="text-polly-gray-dark dark:text-polly-gray-light">Atualize os detalhes do produto.</CardDescription>
        </CardHeader>
        <CardContent>
